import { ICacheContent } from '../interfaces/cache-content.interface';
...
export class OltCacheService {
   ...
   */
  get(key: string, fallback?: Observable<any>, maxAge?: number): Observable<any> | Subject<any> | undefined {
  ...
  private hasValidCachedValue(key: string): boolean {
    if (this.cache.has(key)) {
      const content = this.cache.get(key);
      if (content == null || content.expiry < Date.now()) {
